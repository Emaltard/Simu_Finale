...
void lbm_comm_sync_ghosts_horizontal( lbm_comm_t * mesh, Mesh *mesh_to_process, lbm_comm_type_t comm_type, int target_rank, int x )
{
	...
	{
		case COMM_SEND:
			MPI_Send( Mesh_get_col( mesh_to_process, x ), mesh->height * DIRECTIONS, MPI_DOUBLE, target_rank, 0, MPI_COMM_WORLD);
			break;
		case COMM_RECV:
			MPI_Recv(  Mesh_get_col( mesh_to_process, x ), mesh->height * DIRECTIONS, MPI_DOUBLE, target_rank, 0, MPI_COMM_WORLD,&status);
			break;
		default:
